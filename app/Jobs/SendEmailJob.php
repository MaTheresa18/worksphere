...
class SendEmailJob implements ShouldQueue
{
    ...
    protected function sendEmail(Email $email, EmailAccount $account): void
    {
        ...
        $mailer->send([], [], function (Message $message) use ($email, $account) {
            ...
            // Read Receipt
            if (!empty($email->headers['request_read_receipt'])) {
            ...
            foreach ($email->getMedia('attachments') as $media) {
                if ($media->getCustomProperty('disposition') === 'inline') {
                    $cid = $media->getCustomProperty('content_id');
                    // Use Symfony's embedFromPath directly to avoid mandatory filename in Laravel's embed()
                    // Passing null for name ensures many clients don't show it as a separate attachment.
                    $embeddedCid = $message->getSymfonyMessage()->embedFromPath($media->getPath(), null, $media->mime_type);
                    
                    // Replace our interim CID with the one generated by mailer
                    if ($cid) {
                        $htmlBody = str_replace('cid:' . $cid, $embeddedCid, $htmlBody);
                    }
                } else {
