...
const isAllSelected = computed(() => {
    return (
        logs.value.length > 0 && selectedLogs.value.size === logs.value.length
    );
});

const isIndeterminate = computed(() => {
    return (
        selectedLogs.value.size > 0 &&
        selectedLogs.value.size < logs.value.length
    );
});
...
const logLevels = [
    {
        value: "debug",
        label: "Debug",
        class: "text-gray-600 bg-gray-100 dark:bg-gray-800 dark:text-gray-400",
    },
    {
        value: "info",
        label: "Info",
    ...
    },
    {
        value: "notice",
        label: "Notice",
        class: "text-cyan-600 bg-cyan-100 dark:bg-cyan-900/30 dark:text-cyan-400",
    },
    {
        value: "warning",
        label: "Warning",
        class: "text-orange-600 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-400",
    },
    {
        value: "error",
        label: "Error",
    ...
    },
    {
        value: "critical",
        label: "Critical",
        class: "text-red-700 bg-red-200 dark:bg-red-900/50 dark:text-red-300",
    },
    {
        value: "alert",
        label: "Alert",
        class: "text-purple-600 bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400",
    },
    {
        value: "emergency",
        label: "Emergency",
        class: "text-red-800 bg-red-300 dark:bg-red-900/70 dark:text-red-200",
    },
];

const getLevelStyle = (level) => {
    return (
        logLevels.find((l) => l.value === level)?.class ||
...
const fetchLogs = debounce(async (page = 1) => {
    ...
    try {
        ...
        if (response.data.meta.statistics) {
            statistics.value = response.data.meta.statistics;
        }
...
const downloadLog = () => {
    if (!selectedFile.value) return;
    window.open(
        `/api/system-logs/download?file=${selectedFile.value}`,
        "_blank",
    );
};
...
const exportSelected = () => {
    ...
    const selectedData = logs.value.filter((_, index) =>
        selectedLogs.value.has(index),
    );
    const textContent = selectedData
        .map(
            (log) =>
...
<template>
    <div class="space-y-4">
        <!-- Statistics Cards -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4" v-if="selectedFile">
            <div
                class="bg-(--surface-elevated) rounded-xl border border-(--border-default) p-4"
            >
            ...
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-(--text-primary)">
                            {{ statistics.errors?.toLocaleString() || 0 }}
                        </p>
            ...
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-(--text-primary)">
                            {{ statistics.warnings?.toLocaleString() || 0 }}
                        </p>
                        <p class="text-xs text-(--text-muted)">Warnings</p>
                    </div>
                </div>
            </div>
            <div
            ...
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-(--text-primary)">
                            {{ Object.keys(statistics.envs || {}).length }}
                        </p>
                        <p class="text-xs text-(--text-muted)">Environments</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- Toolbar -->
        <div
            class="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center bg-(--surface-elevated) p-4 rounded-xl border border-(--border-default)"
        >
                ...
                <div class="flex items-center gap-2">
                    ...
                    >
                        <RefreshCw
                            class="w-4 h-4"
                            :class="{
                                'animate-spin': autoRefresh || isLoading,
                            }"
                        />
                    </Button>
                    <Button
        ...
        <!-- Log Viewer -->
        <div
            class="bg-(--surface-elevated) rounded-xl border border-(--border-default) overflow-hidden flex flex-col"
        >
            <!-- Logs List -->
            <div
                class="flex-1 overflow-y-auto min-h-[300px] max-h-[70vh] relative"
            >
                <table class="w-full text-left text-sm font-mono">
                    <thead
                        class="bg-(--surface-secondary) text-(--text-secondary) border-b border-(--border-default) sticky top-0 z-10"
                    >
                        <tr>
                            ...
                            <th
                                class="px-4 py-3 w-40 cursor-pointer hover:bg-(--surface-tertiary) transition-colors group"
                            ...
                            <th
                                class="px-4 py-3 w-24 cursor-pointer hover:bg-(--surface-tertiary) transition-colors group hidden md:table-cell"
                                @click="handleSort('env')"
                            >
                                <div class="flex items-center gap-1">
                                    Env
                                    <component
                            ...
                            <th
                                class="px-4 py-3 w-24 cursor-pointer hover:bg-(--surface-tertiary) transition-colors group"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <td
                                        class="px-4 py-3 text-[var(--text-secondary)] whitespace-nowrap"
                                    >
                                        {{ formatTime(log.timestamp) }}
                                    </td>
                                    <td
                                        class="px-4 py-3 text-[var(--text-secondary)] hidden md:table-cell"
                                    >
                                        <span
                                            class="px-2 py-0.5 rounded textxs bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
                                        >
                                            {{ log.env }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span
                                            :class="[
                                                'px-2 py-0.5 rounded text-xs font-medium uppercase border border-transparent',
                                ...
                                <tr v-if="expandedLogs.has(index)">
                                    <td
                                        colspan="5"
                                        class="px-4 py-4 bg-[var(--surface-tertiary)]/30 border-b border-[var(--border-default)]"
                                    >
                                        <Transition name="expand" appear>
                                            <div class="space-y-4">
                                                ...
                                                <div v-if="log.stack_trace">
                                                    ...
                                                    <pre
                                                        class="text-xs bg-[var(--surface-primary)] p-4 rounded-lg border border-[var(--border-default)] overflow-x-auto overflow-y-auto max-h-96 text-[var(--text-secondary)]"
                                                        >{{
                                                            log.stack_trace
                                                        }}</pre
                                                    >
            ...
            <!-- Pagination -->
            <div
                class="px-4 py-3 border-t border-[var(--border-default)] bg-[var(--surface-elevated)]"
            >
